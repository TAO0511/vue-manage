<template>
  <div class="templates">
    <div v-if="isShowType('list')">
      ...
      <div class="table-list">
        <el-table :data="tableData" stripe style="width: 100%">
          ...
          <el-table-column prop="parent.name" :label="$t('subClassify')"></el-table-column>
          <el-table-column prop="name" :label="$t('classifyName')"></el-table-column>
    ...
    <CategoryEdit v-if="isShowType('add')" @cancel="cancelAdd" @sure="query()" :data="editItem" />
  </div>
</template>
  ...
  methods: {
    ...
    handleDelete (deleteItem) {
      this.$confirm(`确定删除【${deleteItem.name}】吗?`, '提示', {
        ...
        category.delete({ id: deleteItem._id }).then(res => {
          console.log(res)
          this.query()
        })
      }).catch(() => {
